    ...
    <tbody class="markup">
        ...
        <tr>
            <td data-title="ID Product">
                ...
                @else
                <a id="delete" value="{{ $item->sales_purchase_detail_item_product_id }}"
                    href="{{ route(config('module').'_delete', ['code' => $item->sales_purchase_detail_id, 'detail' => $item->sales_purchase_detail_item_product_id ]) }}"
                    class="btn btn-danger btn-xs btn-block delete-{{ $item->sales_purchase_detail_item_product_id }}">
                    {{ $item->sales_purchase_detail_item_product_id }}
                </a>
                @endif
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->sales_purchase_detail_item_product_id }}" name="temp_id[]">
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->sales_purchase_detail_item_product_id }}"
            ...
            <td data-title="Product">
                ...
                <div class="input-group input-group-sm">
                    <input type="text" tabindex="{{ $loop->iteration }}3" name="detail[{{ $loop->index }}][temp_desc]" value="{{ $item['temp_desc'] ?? $item->sales_purchase_detail_discount_name }}" class="form-control">
                    <span class="input-group-btn">
                        <button class="btn btn-default" type="button">disc</button>
                    </span>
                </div>

                <textarea placeholder="notes" rows="4" tabindex="{{ $loop->iteration }}5" class="form-control temp_notes simple" name="detail[{{ $loop->index }}][temp_notes]">{{ $item['temp_notes'] ?? $item->sales_purchase_detail_item_product_description }}</textarea>
            </td>
            <td data-title="Qty" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}1" name="detail[{{ $loop->index }}][temp_qty]"
                    ...
                    value="{{ $item['temp_qty'] ?? $item->sales_purchase_detail_qty }}">
                ...
                <input type="text" tabindex="{{ $loop->iteration }}4" name="detail[{{ $loop->index }}][temp_disc]" class="form-control input-sm text-right number temp_disc"
                    value="{{ $item['temp_disc'] ?? $item->sales_purchase_detail_discount_percent }}">
            </td>
            <td data-title="Price" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}2"  name="detail[{{ $loop->index }}][temp_price]"
                    ...
                    value="{{ $item['temp_price'] ?? $item->sales_purchase_detail_price }}">

                <input type="text" name="detail[{{ $loop->index }}][temp_potongan]"
                    class="form-control input-sm text-right number temp_potongan"
                    value="{{ $item['temp_potongan'] ?? $item->sales_purchase_detail_discount_value }}">
            </td>
            <td data-title="Total" class="text-right col-lg-1">
                <input type="text" readonly name="detail[{{ $loop->index }}][temp_total]"
                    ...
                    value="{{ $item['temp_total'] ?? $item->sales_purchase_detail_total }}">
            </td>
        </tr>
        @endforeach
        @endisset
    ...
    <tbody>
        <tr>
            ...
            <td data-title="" class="text-right">
                {!! Form::text('sales_order_sum_product', $model->sales_purchase_sum_product, ['id' => 'before_discount',
        ...
        <tr style="margin-bottom: 20px;">
            ...
            <td data-title="Description" class="text-left col-md-4">
                {!! Form::textarea('sales_order_discount_name', $model->sales_purchase_discount_name, ['id' => 'grand_discount_description', 'class' =>
                'form-control', 'rows' => 2, 'tabindex' => 500]) !!}
            </td>
            <td data-title="Value" class="text-right col-md-1">
                {!! Form::text('sales_order_discount_percent', $model->sales_purchase_discount_percent, ['id' => 'grand_discount_value', 'placeholder' =>
                'Dalam %' ,'class' => 'number form-control text-right', 'tabindex' => 501]) !!}
            </td>
            <td data-title="Price" class="text-right col-md-1">
                {!! Form::text('sales_order_discount_value', $model->sales_purchase_discount_value, ['id' => 'grand_discount_price',
                'readonly', 'class' => 'number form-control text-right', 'tabindex' => 502]) !!}
            </td>
            <td data-title="Total" class="text-right col-md-1">
                {!! Form::text('sales_order_sum_discount', $model->sales_purchase_sum_discount, ['id' => 'grand_discount_total',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
        </tr>
        <tr style="margin-bottom: 20px;">
            ...
            <td data-title="Description" class="text-left col-md-4">
                {{ Form::select('sales_order_tax_id', $tax, $model->sales_purchase_tax_id, ['class'=> 'form-control', 'id' => 'grand_tax_id', 'tabindex' => 503]) }}
            </td>
            <td data-title="Value" class="text-right col-md-1">
                {!! Form::text('sales_order_tax_percent', $model->sales_purchase_tax_percent, ['id' => 'grand_tax_value', 'placeholder' => 'Dalam %'
                ,'class' => 'number form-control text-right', 'tabindex' => 504]) !!}
            </td>
            <td data-title="Price" class="text-right col-md-1">
                {!! Form::text('sales_order_tax_value', $model->sales_purchase_tax_value, ['id' => 'grand_tax_price',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
            <td data-title="Total" class="text-right col-md-1">
                {!! Form::text('sales_order_sum_tax', $model->sales_purchase_sum_tax, ['id' => 'grand_tax_total',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
        </tr>
        <tr>
            ...
            <td data-title="" class="text-right">
                {!! Form::text('sales_order_sum_total', $model->sales_purchase_sum_total, ['id' => 'grand_total',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
        </tr>
    </tbody>
