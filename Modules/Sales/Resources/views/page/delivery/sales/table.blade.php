    ...
    <tbody class="markup">
        ...
        <tr>
            <td data-title="ID Product">
                ...
                @else
                <a id="delete" value="{{ $item->sales_delivery_detail_item_product_id }}"
                    href="{{ route(config('module').'_delete', ['code' => $item->sales_delivery_detail_id, 'detail' => $item->sales_delivery_detail_item_product_id ]) }}"
                    class="btn btn-danger btn-xs btn-block delete-{{ $item->sales_delivery_detail_item_product_id }}">
                    {{ $item->sales_delivery_detail_item_product_id }}
                </a>
                @endif
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->sales_delivery_detail_item_product_id }}" name="temp_id[]">
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->sales_delivery_detail_item_product_id }}"
            ...
            <td data-title="Price" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}2"  name="detail[{{ $loop->index }}][temp_price]"
                    ...
                    value="{{ $item['temp_price'] ?? $item->sales_delivery_detail_price }}">
            </td>
            <td data-title="Total" class="text-right col-lg-1">
                <input type="text" readonly name="detail[{{ $loop->index }}][temp_total]"
                    ...
                    value="{{ $item['temp_total'] ?? $item->sales_delivery_detail_total }}">
            </td>
        </tr>
        @endforeach
        @endisset
    ...
    <tbody>
        <tr>
            ...
            <td data-title="" class="text-right">
                {!! Form::text('sales_delivery_sum_product', null, ['id' => 'before_discount',
        ...
        <tr style="margin-bottom: 20px;">
            ...
            <td data-title="Description" class="text-left col-md-4">
                {!! Form::textarea('sales_delivery_discount_name', null, ['id' => 'grand_discount_description', 'class' =>
                'form-control', 'rows' => 2, 'tabindex' => 500]) !!}
            </td>
            <td data-title="Value" class="text-right col-md-1">
                {!! Form::text('sales_delivery_discount_value', null, ['id' => 'grand_discount_value', 'placeholder' =>
                'Input Value' ,'class' => 'number form-control text-right', 'tabindex' => 501]) !!}
            </td>
            <td data-title="Price" class="text-right col-md-1">
                {!! Form::text('sales_delivery_discount_value', null, ['id' => 'grand_discount_price',
                'readonly', 'class' => 'number form-control text-right', 'tabindex' => 502]) !!}
            </td>
            <td data-title="Total" class="text-right col-md-1">
                {!! Form::text('sales_delivery_sum_discount', null, ['id' => 'grand_discount_total',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
        </tr>
        ...
        <tr>
            ...
            <td data-title="" class="text-right">
                {!! Form::text('sales_delivery_sum_total', null, ['id' => 'grand_total',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
        </tr>
    </tbody>
