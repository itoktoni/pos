...
<head>
    ...
    *:after {
    ...
    <title>Purchase_{{ $master->sales_purchase_id ?? '' }}</title>
</head>
            ...
            <table cellpadding="" 5 cellspacing="0" width="100%">
                <tr>
                    <td align='left' colspan='8' valign='middle'>
                        <h1 id="headline">
                            PURCHASE ORDER ( {{ $master->sales_purchase_id ?? '' }} )
                        </h1>
                    </td>
                </tr>
                ...
                <tr class="contact">
                    ...
                    <td colspan='4' class="message">
                        Order Date : {{ $master->sales_purchase_date->format('d F Y') }}
                        <br>
                        {{ $master->sales_purchase_notes_internal ?? '' }}
                    </td>
                </tr>
                ...
                <tr class="item">
                    ...
                    <td class="price">
                        {{ Helper::createRupiah($item->sales_purchase_detail_price) ?? '' }}
                    </td>
                    <td class="qty">
                        {{ $item->sales_purchase_detail_qty ?? '' }}
                    </td>
                    <td class="total">
                        @if($item->sales_purchase_detail_discount_percent)
                        <span>
                            Disc : {{ $item->sales_purchase_detail_discount_percent }}%
                        </span>
                        <span>
                            - {{ Helper::createRupiah($item->sales_purchase_detail_discount_value) }}
                        </span>
                        @endif
                        {{ Helper::createRupiah($item->sales_purchase_detail_total) ?? '' }}
                    </td>
                </tr>
                @endforeach
                <tr class="total_product">
                    ...
                    <td class="qty">
                        {{ $detail->sum('sales_purchase_detail_qty') ?? '' }}
                    </td>
                    <td class="total">
                        {{ Helper::createRupiah($detail->sum('sales_purchase_detail_total')) ?? '' }}
                    </td>
                </tr>
                @if (!empty($master->sales_purchase_discount_percent))
                <tr class="total_discount">
                    <td class="product" colspan="7">
                        {{ $master->sales_purchase_discount_name ?? '' }}
                        Total Discount
                    </td>

                    <td class="total">
                        -{{ Helper::createRupiah($master->sales_purchase_discount_value) ?? '' }}
                    </td>
                </tr>
                @endif
                @if (!empty($master->sales_purchase_tax_percent))
                <tr class="total_discount">
                    <td class="product" colspan="6">
                        {{ $master->tax->finance_tax_name ?? '' }}
                    </td>
                    <td class="qty">
                        {{ $master->sales_purchase_tax_percent ?? '' }}%
                    </td>
                    <td class="total">
                        {{ Helper::createRupiah($master->sales_purchase_tax_value) ?? '' }}
                    </td>
                </tr>
                @endif
                <tr class="total_sumary">
                    ...
                    <td class="total">
                        {{ Helper::createRupiah($master->sales_purchase_sum_total) ?? '' }}
                    </td>
                </tr>
