...
<table id="transaction" class="table table-no-more table-bordered table-striped">
    <thead>
        <tr>
            <th class="text-left col-md-2">ID</th>
    ...
    <tbody>
        ...
        <tr>
            <td data-title="ID Product">{{ $product->product->item_product_id ?? '' }}</td>
            <td data-title="Product">
                {{ $product->product->item_product_name ?? '' }}
            ...
            <td data-title="Price" class="text-right col-lg-1">
                <input type="text" name="detail[{{ $loop->index }}][temp_price]"
                    ...
                    value="{{ !empty($product->purchase_detail_price_order) ? $product->purchase_detail_price_order : 0 }}">
            </td>
            <td data-title="Qty" class="text-right col-lg-1">
                <input type="text" name="detail[{{ $loop->index }}][temp_qty]"
                    class="form-control text-right number temp_qty" value="{{ $product->purchase_detail_qty_order }}">
            </td>
            <td data-title="Total" class="text-right col-lg-1">
                <input type="text" readonly name="detail[{{ $loop->index }}][temp_total]"
                    ...
                    value="{{ $product->purchase_detail_total_order }}">
            </td>
            <td data-title="Action">
                <input type="hidden" value="{{ $product->purchase_detail_item_product_id }}" name="temp_id[]">
                <input type="hidden" value="{{ $product->purchase_detail_item_product_id }}" name="detail[{{ $loop->index }}][temp_id]">
                <input type="hidden" value="{{ $product->purchase_detail_item_product_id }}"
                    name="detail[{{ $loop->index }}][temp_product]">
                
                <button id="delete" value="{{ $product->purchase_detail_item_product_id }}" type="button"
        ...
        <tr>
            ...
            <td data-title="Qty" class="text-right col-lg-1">
                <input type="text" name="detail[{{ $loop->index }}][temp_qty]"
                    class="form-control text-right number temp_qty" value="{{ $product['temp_qty'] }}">
            </td>
            <td data-title="Total" class="text-right col-lg-1">
                <input type="text" readonly name="detail[{{ $loop->index }}][temp_total]"
            ...
            <td data-title="Action">
                ...
                <input type="hidden" value="{{ $product['temp_product'] }}"
                    name="detail[{{ $loop->index }}][temp_product]">
