                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4 {{ $errors->has('product') ? 'has-error' : ''}}">
                        <select class="form-control col-md-4" id="product" name="product">
                            ...
                            <option value="{{ $value->item_product_id.'#'.floatval($value->item_product_buy) }}">
...
$(function() {
    ...
    function addDetail(e) {
        ...
        var input_counter = $('input[name=temp_counter]');
        ...
        var counter = numeral(input_counter.val()).value();
        ...
        if (product_value) {
            ...
            if (product_id) {
                ...
                var markup = "<tr><td data-title='ID Product'>" + id + "</td><td data-title='Product'>" + name +
                    "</td><td data-title='Price' class='text-right col-lg-1'><input name=detail[" + counter +
                    "][temp_price] class='form-control text-right number temp_price' value='" + real_price +
                    "'></td><td data-title='Qty' class='text-right col-lg-1'><input class='form-control text-right number temp_qty' name=detail[" +
                    counter + "][temp_qty] value='" + value_qty +
                    "'></td><td data-title='Total' class='text-right col-lg-1'><input type='text' name=detail[" +
                    counter +
                    "][temp_total] readonly class='form-control text-right money temp_total' value='" + numeral(
                        total).format('0,0') + "'></td><td data-title='Action'><button id='delete' value='" +
                    id +
                    "' type='button' class='btn btn-danger btn-block'>Delete</button></td><input type='hidden' value=" +
                    id + " name=detail[" + counter + "][temp_id]><input type='hidden' value=" + product_id +
                    " name=detail[" + counter + "][temp_product]><input type='hidden' value='" + name +
                    "' name=detail[" + counter + "][temp_name]><input type='hidden' value='" + id +
                    "' name=temp_id[]></tr>";
                $("table tbody").append(markup);
                sumTotal();
                counter++;
                $('#temp_counter').val(counter);
